...
**Version:** `1.1.2`
**Current server target:** Minecraft 1.21.1 · Arclight · Pixelmon 9.3.16

> **1.21.1 port note.** This branch is the server-tested 1.21.1 line. It keeps
> the same user-facing Hatchery behavior, but the Pixelmon/Arclight integration
> was adjusted for the 1.21.1 runtime (`v1_21_R1` CraftBukkit package, current
> Pixelmon party storage behavior, and Pixelmon 9.3.x Day Care blocks).
>
> **Block change in 1.1.1.** Pixelmon 9.x no longer ships `ranch_block`; the
> modern equivalent is `pixelmon:<colour>_day_care` (16 dyed variants).
> Hatchery binds to all 16 colours by default and short-circuits Pixelmon's own
> Day Care interaction. See `CONFIG.md` for the full list and how to narrow it.
...
| **[PLAN.md](./PLAN.md)**                           | Devs    | Original implementation notes and package map |
...
Pixelmon and Minecraft server jars are compile-only local dependencies and are
not committed. See [`libs/README.md`](./libs/README.md) before building.

---

## What's in 1.1.2

- **1.21.1 runtime port** — sprite conversion now uses the 1.21.1
  CraftBukkit package and direct Pixelmon APIs where the new runtime is stable.
- **Party-removal dupe guard** — placing a parent in the daycare now clears
  both Pixelmon's active and original party storage layers, sends the storage
  update packets, and refuses placement if the same Pokemon UUID is still in
  the party afterward.
- **Egg held-item cleanup** — eggs generated by Hatchery have their held item
  cleared so a parent-held item cannot be cloned onto the egg.
- **Stale parent cleanup** — if an older daycare record already contains a
  Pokemon that is also still in the player's party, shift-click retrieval
  clears the daycare copy instead of duplicating the Pokemon again.
...
Original implementation notes live in [`PLAN.md`](./PLAN.md).
